...
mkdir -p $temp_dir
cd $temp_dir
extracted_dir=${temp_dir}/redis-plus-plus-${version}
if [ ! -e ${extracted_dir} ] ; then
  tar_filename=${version}.tar.gz
  if [ ! -e ${tar_filename} ] ; then
    curl -fsSL https://github.com/sewenew/redis-plus-plus/archive/refs/tags/${tar_filename} -o ${tar_filename}
  fi
